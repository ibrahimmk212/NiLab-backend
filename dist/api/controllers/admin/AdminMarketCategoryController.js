...
class AdminVendorCategoryController {
    constructor() {
        this.create = (0, async_1.asyncHandler)(async (req, res, next) => {
            const newCategory = await MarketCategoryService_1.default.create(req.body);
            if (!newCategory)
        ...
        this.update = (0, async_1.asyncHandler)(async (req, res, next) => {
            const { id } = req.params;
            const updated = await MarketCategoryService_1.default.update(id, req.body);
            if (!updated)
                res.status(constants_1.STATUS.BAD_REQUEST).send({
                    success: false,
        ...
        this.getSingle = (0, async_1.asyncHandler)(async (req, res) => {
            ...
            if (!product) {
                res.status(constants_1.STATUS.NOT_FOUND).send({
                    success: false,
